...
func (q *Queue) Bind(af_family int) error {
	 ...
	 */
	_ = C.nfq_bind_pf(q.c_h, C.uint16_t(af_family))
	return nil
}
...
func (q *Queue) Unbind(af_family int) error {
	...
	rc := C.nfq_unbind_pf(q.c_h, C.uint16_t(af_family))
	if rc < 0 {
...
func (q *Queue) CreateQueue(queue_num int) error {
	...
	q.c_qh = C.nfq_create_queue(q.c_h, C.uint16_t(queue_num), (*C.nfq_callback)(C.c_nfq_cb), unsafe.Pointer(q))
	if q.c_qh == nil {
...
func (q *Queue) SetMode(mode uint8) error {
	...
	C.nfq_set_mode(q.c_qh, C.uint8_t(mode), 0xffff)
	return nil
}
...
func (q *Queue) SetQueueMaxLen(maxlen uint32) error {
	...
	C.nfq_set_queue_maxlen(q.c_qh, C.uint32_t(maxlen))
	return nil
}
...
func (p *Payload) SetVerdict(verdict int) error {
	...
	C.nfq_set_verdict(p.c_qh, C.uint32_t(p.Id), C.uint32_t(verdict), 0, nil)
	return nil
}
